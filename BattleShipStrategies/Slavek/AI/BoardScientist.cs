...
{
    public static PreparedMap StableNemesis(GameSetting setting, IGameStrategy strategy)
    {
        ...
        /*Game tgame = new Game(paster, setting);
        for (int i = 0; i < 10; i++)
            Console.WriteLine(tgame.SimulateGame(strategy));*/
    ...
    public static PreparedMap StableSmart(GameSetting setting, Dictionary<IGameStrategy, double> enemies,
        Dictionary<IGameStrategy, double> allies)
    {
        IBoardCreationStrategy boardStrategy = new SmartRandomBoardCreationStrategy();
        BoatPaster paster = new BoatPaster();
        int boatTileCount = 0;
